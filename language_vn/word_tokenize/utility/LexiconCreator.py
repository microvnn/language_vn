...
def createLexicon(corpusFilePath, fullLexicon):
    ...
    for word in wordTagCounter:
        ...
        if re.search(r"[0-9]+", word) != None:
            ...
            if len(decodedWord) >= 4:
                ...
                add2WordTagFreqDict(suffix.encode("utf-8"),
                                    tag, suffixDictCounter)
    ...
    tagCounter_Alphabet = OrderedDict(
        sorted(tagCounter_Alphabet.iteritems(), key=itemgetter(1), reverse=True))
    tagCounter_CapitalizedWord = OrderedDict(
        sorted(tagCounter_CapitalizedWord.iteritems(), key=itemgetter(1), reverse=True))
    tagCounter_Numeric = OrderedDict(
        sorted(tagCounter_Numeric.iteritems(), key=itemgetter(1), reverse=True))
